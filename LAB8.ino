...
void setup() {
  ...
  pinMode(Boton1, INPUT_PULLUP); //Boton 1 como Pull-up
  ...
  String text1 = "Lab8 D2";
  LCD_Print(text1, 110, 110, 2, 0xffff, 0x0000);

  cont = 0; //Definir el contador en 0
}
//***************************************************************************************************************************************
// Loop
//***************************************************************************************************************************************
void loop() {
  int Bot1 = digitalRead(Boton1);             //Lectura del Boton

  if(Bot1 == LOW){                          //Si esta en LOW desplegar el menu
   if(cont >= 2){
    cont = 0;
    }
   else{
...
switch(cont){
      case 0:                           //ASCII "1" envía el contenido de H.txt
      FSD("f1.txt");
      break;
      
      case 1:                          //ASCII "2" envía el contenido de F.txt
      FSD("f2.txt");
      break;
      
      case 2:             //ASCII "3" envía el contenido de jhin.txt
      FSD("f3.txt");
      break;  
    }
}
...
//***************************************************************************************************************************************
void FSD(char x[]){
  ARCH = SD.open(x, FILE_READ);  //Open image
  int hex = 0;
  int v1 = 0;
  int v2 = 0;
  int mapar = 0;
  int vert = 0;
  if(ARCH){  //If the image file opened
    Serial.println("Leyendo el archivo...");
    while(vert <= 239){
      mapar = 0;
      while (mapar<640){
        hex = ARCH.read();
        if(hex==120){    //Check for the x of 0xFF hex
          v1 = ARCH.read();  //Read the next 2 characters which will be the hex code
          v2 = ARCH.read();
          v1 = ASCII_Hex(v1);
          v2 = ASCII_Hex(v2);
          maps[mapar] = (v1*16)+v2;
